    ...
    <div class="container">
      <div class="wrapper">
        ...
        <div class="buttons">
          ...
          <span class="button btnAddMatricula" @click="addMatricula()">Salvar</span>
        </div>
      </div>
    </div>
  </div>
  ...
  methods: {
    ...
    addMatricula() {
      ...
      if(!data.cpf)
        ...
        self.$store.dispatch("matriculas/addMatricula", data)
          .then(() => {
            self.$router.push('/matriculas/');
            alert('Matricula adicionada com sucesso!');
          })
          .catch(e => {
            alert('Não foi possível adicionar a matrícula, entre em contato com o suporte.');
          });
      }
    },
    myChangeEvent(cursoId) {
      this.cursoId = parseInt(cursoId);
    },
    mySelectEvent(event) {}
  }
}
